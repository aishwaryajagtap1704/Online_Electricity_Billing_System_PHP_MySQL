...
function hasNewBills() {
    global $con;
    // Implement logic to check if there are new bills in "Due"
    require_once('../Includes/config.php'); // Adjust the path as necessary
    require_once('../Includes/session.php'); // Adjust the path as necessary
    
    $id = $_SESSION['uid'];
    $query = "SELECT COUNT(*) FROM bill WHERE uid = $id AND status = 'PENDING'";
    $result = mysqli_query($con, $query);
